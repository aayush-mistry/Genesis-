...
export const WorldController = {
  // --- World ---
  getWorld: async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  createWorld: async (request: FastifyRequest, reply: FastifyReply) => {
    const { name, description, seed } = request.body as any;
    const world = worldService.engine.worldManager.createWorld(name, description, seed);
    return world;
  },

  deleteWorld: async (request: FastifyRequest, reply: FastifyReply) => {
    worldService.engine.worldManager.resetWorld();
    return { success: true };
  },

  // --- Regions ---
  getRegions: async (request: FastifyRequest, reply: FastifyReply) => {
    return worldService.engine.regionManager.getAllRegions();
  },

  createRegion: async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  // --- Districts ---
  getDistricts: async (request: FastifyRequest, reply: FastifyReply) => {
    return worldService.engine.districtManager.getAllDistricts();
  },

  createDistrict: async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  // --- Buildings ---
  getBuildings: async (request: FastifyRequest, reply: FastifyReply) => {
    return worldService.engine.buildingManager.getAllBuildings();
  },

  createBuilding: async (request: FastifyRequest, reply: FastifyReply) => {
