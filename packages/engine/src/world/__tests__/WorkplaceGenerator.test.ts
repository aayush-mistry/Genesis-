...
describe('WorkplaceGenerator', () => {
  ...
  it('TEST 14 & 15 & 16 & 17: Generates workplaces based on world state', () => {
    ...
    const city1 = worldEngine.cityManager.createCity({ name: 'C1', population: 0, coordinates: { x:0, y:0 }, area: 100, regionId: region1.id, districtCount: 1, buildingCount: 1 });
    worldEngine.regionManager.addCity(region1.id, city1.id);

    const district1 = worldEngine.districtManager.createDistrict({ name: 'D1', type: 'COMMERCIAL' as any, cityId: city1.id, buildingIds: [] });
    worldEngine.cityManager.addDistrict(city1.id, district1.id);

    const building1 = worldEngine.buildingManager.createBuilding({ name: 'B1', type: 'OFFICE' as any, coordinates: { x:0, y:0 }, capacity: 10, status: 'OK', districtId: district1.id, roomIds: [] });
